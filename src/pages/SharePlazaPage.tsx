  ...
  return (
    <div className="plaza-page">
      <header className="plaza-header">
        ...
        <p className="plaza-subtitle">다른 사용자가 공유한 여행 일정을 둘러보고 내 여행으로 끌어올 수 있습니다.</p>
                ...
                return (
                  <article key={entry.id} id={`plaza-row-${entry.id}`} className="plaza-board-row">
                    <div className="plaza-board-meta">
                      ...
                      <time className="plaza-board-date" dateTime={new Date(entry.listedAt).toISOString()}>
                        {formatDate(entry.listedAt, {
                          year: 'numeric',
                          month: 'short',
                          day: 'numeric',
                        }, locale)}
        ...
        {tab === 'map' && (
          <section className="plaza-map-section" aria-label="전국 지도">
            ...
            {plazaMarkers.length === 0 && !loading && (
              <p className="plaza-map-empty">지도에 표시할 위치 정보가 있는 공유 여행이 없습니다.</p>
            )}
          </section>
        )}
      </main>
